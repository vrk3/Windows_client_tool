...
from .owners import (
    OwnerResolver, is_available as owners_available, resolve_sampled_owners,
)
...
class Scanner:
    """Engine selection and orchestration (spec 3.4)."""
    ...
    def __init__(self, target: str, filters: FilterSet | None = None,
        ...
        # Off by default: the walk pays a security call per file for this, and
        # only the Users view and the Owner column spend it (spec 5.8).
        self.collect_owners = collect_owners
        self._resume = threading.Event()
        self._resume.set()

    def _owner_resolver(self):
        if not self.collect_owners:
            return None
        ok, _why = owners_available()
        return OwnerResolver() if ok else None
    ...
    def scan(self, on_batch=None, should_cancel=None) -> ScanResult:
        ...
        if engine == "mft":
            ...
            if scanner.truncated:
                ...
                error_count = 1
            if self.collect_owners:
                # The MFT records a security id, not the descriptor behind it.
                # One sampled file per distinct id recovers the account name
                # for the whole bucket -- a handful of calls, not one per file.
                resolve_sampled_owners(store, OwnerResolver())
        else:
            ...
            scanner = WalkScanner(self.target, bytes_per_cluster=cluster,
                                  ...
                                  owner_resolver=self._owner_resolver())
            scanner.scan(store, on_batch=on_batch, should_cancel=should_cancel,
                         wait_if_paused=self._wait_if_paused)
            root = scanner.root
            mft_extents = 0
